    ...
    def createEParams(self):
        for tag in self.tags:
            ...
            for ob in self.obsList:
                self.eParams[tag][ob] = [0.0, 0.0]  # [weight, average weight] for each tag - ob
    ...
    def createTParams(self):
        ...
        for p_tag in p_tagsList:
            ...
            for c_tag in c_tagsList:
                self.tParams[p_tag][c_tag] = [0.0, 0.0] # [weight, average weight] for each tag - ob
    ...
    def trainPerceptron(self, epochs=3, lr=1.0):
        ...
        for t in list(range(epochs)):
            ...
            for line in open(self.filePath, 'r'):
                ...
                if lineAsList:
                    lineAsList = lineAsList.rsplit(' ', 1)
                    word, tag = lineAsList[0], lineAsList[1]  
...
if __name__ == '__main__':
    ...
    parser.add_argument('-traindata', type=str, dest='dataset', help='Dataset to train perceptron', required=True, default='EN/train')
    parser.add_argument('-testdata', type=str, dest='testdata', help='Dataset to train perceptron', required=True, default='EN/train')
    ...
    output_data_path
    test_output = "test_output/test.p5.out"
    ...
    model.predict(args.testdata,test_output)
